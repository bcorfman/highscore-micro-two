...
async def get_high_scores(session: AsyncSession = Depends(db_setup.get_session)):
    result = await session.execute(
        select(HighScore.initials, HighScore.score).order_by(HighScore.score.desc())
    )
...
async def add_score_to_list(
    initials: str, score: int, session: AsyncSession = Depends(db_setup.get_session)
):
    ...
    if len(initials) > 0 and score >= 0:
        ...
        if total_rows >= 10:
            ...
            await session.execute(delete(HighScore).where(HighScore.score < last_score))
            await session.commit()
    return hs
...
async def clear_high_score_list(session: AsyncSession = Depends(db_setup.get_session)):
    ...
    result = await session.execute(select(HighScore))
    high_scores = result.all()
    return [HighScoreBase(initials=hs.initials, score=hs.score) for hs in high_scores]
